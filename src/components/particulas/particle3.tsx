        ...
        <AnimatePresence mode="wait">
            <motion.section
                ...
                style={{
                    ...
                    // Sin overflow-hidden — nada se corta
                }}
                initial={{ scale: 1.5, opacity: 0, y: -20 }}
                animate={{ scale: 1, opacity: 1, y: 0 }}
                exit={{ scale: 1.5, opacity: 0, y: -20 }}
            >
                {/* Marco: solo decorativo, pointer-events none, no afecta layout */}
                <img
                    src={marco}
                    ...
                    style={{
                        ...
                        width: '100%',
                        height: '100%',
                        objectFit: 'fill',   // se estira para cubrir sin recortar
                        opacity: 0.6,
                        pointerEvents: 'none',
                    }}
                />
...
interface NaveProps {
    ...
    naveRef: React.RefObject<HTMLSpanElement>;
}
...
function Nave({ mouseXRef, naveRef }: NaveProps) {
    ...
    return (
        ...
        >
            <img src={nave} style={{ width: '100%', height: '100%', objectFit: 'contain' }} />
        </span>
    );
}
...
function BreakItem({ mouseXRef, naveRef }: BreakItemProps) {
    ...
    const spawnX = useRef(mouseXRef.current);
    const spawnY = useRef(window.innerHeight - 100);
    ...
    const checkCollision = () => {
        ...
        document.querySelectorAll('.bounceItem').forEach((item) => {
            ...
            if (overlapX && overlapY) {
                ...
                sfx.play().catch(() => {});
                setTimeout(() => { sfx.pause(); sfx.currentTime = 0; }, 1000);
            }
        });
    };
